...
class JPANet(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if final_attention:
            ...
            self.total = Attention(input_dim=final_arch[0], mlp_arch=final_arch[1:], n_heads=n_heads, dropout=dropout)
        else:
        ...
        self.output=MLP(arch=[final_arch[-1],2],out_activation=torch.nn.LogSoftmax(dim=1), dropout=None)
        ...
        for epoch in epoch_loop:
            ...
            with torch.inference_mode():
                ...
                classified_train=(torch.argmax(
                        self(self.mu_train[idx],
                             self.nu_train[idx],
                             self.jet_train[idx]), axis=1)== self.y_train.squeeze()[idx]
                            ).sum()/self.y_test.shape[0]
    ...
    def graph(self):
        model_graph = draw_graph(self, input_data=[
                                 self.mu_test[:self.batch_size],
                                 self.nu_test[:self.batch_size],
                                 self.jet_test[
                                     :self.batch_size]],
