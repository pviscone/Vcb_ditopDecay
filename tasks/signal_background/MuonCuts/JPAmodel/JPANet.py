...
class JPANet(torch.nn.Module):
    ...
    def predict(self,dataset,bunch=1):
        ...
        with torch.inference_mode():
            ...
            for mu_bunch,nu_bunch,jet_bunch,mass_bunch,secondLept_bunch,y_bunch in loader(bunch_size,dataset.data["Lepton"],dataset.data["MET"],dataset.data["Jet"],dataset.data["Masses"],dataset.data["SecondLept"],dataset.data["label"]):
                mu_bunch = mu_bunch.to(device,non_blocking=True)
                nu_bunch = nu_bunch.to(device,non_blocking=True)
                jet_bunch = jet_bunch.to(device,non_blocking=True)
                secondLept_bunch = secondLept_bunch.to(device,non_blocking=True)
