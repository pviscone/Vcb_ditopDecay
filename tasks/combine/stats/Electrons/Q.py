...
new=uproot.open("new.root")["Electrons"]
old=uproot.open("old.root")["Electrons"]
...
mplhep.histplot(new["semiLeptEle"],ax=ax[0],color="red",ls="--",label="new_bkg")
mplhep.histplot(new["signalEle"],ax=ax[0],color="green",label="new_sig",ls="--")
...
x_new=new["semiLeptEle"].to_hist().axes[0].centers


ratio_old=old["signal"].values()**2/(bkg.values()+old["signal"].values())

ratio_new=new["signalEle"].values()**2/(new["semiLeptEle"].values()+new["signalEle"].values())
