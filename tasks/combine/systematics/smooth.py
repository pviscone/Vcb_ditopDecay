...
def smooth_plot(file,region,sample,syst,s=3):
    ...
    plt.figure()
    plt.plot(x, ratioUp,linestyle="--", color="goldenrod", label="ratioUp")
    plt.plot(x, BSpline(*smoothed_ratioUp)(x), color="red",label="smoothedUp")
    plt.plot(x, ratioDown,linestyle="--",color="dodgerblue", label="ratioDown")
    ...
    os.makedirs(f"smooth/{region}/{sample}",exist_ok=True)
    plt.savefig(f"smooth/{region}/{sample}/{syst}.png")
    plt.close()
    
def smooth_hist(file_in,file_out,region,sample,syst,s=5):
    syst=syst.split("Up")[0].split("Down")[0]
    temp=uproot.recreate("temp.root")
    
    hUp=file_in[f"{region}/{sample}_syst_{syst}Up"].to_numpy()
    h=file_in[f"{region}/{sample}"].to_numpy()
    hDown=file_in[f"{region}/{sample}_syst_{syst}Down"].to_numpy()
    ...
    for sample in samples:
        ...
        for syst in systs:
            if syst in systs_to_smooth:
                ...
                #smooth_plot(f,region,sample,syst,s=5)
                smooth_hist(f,out,region,sample,syst,s=5)
            else:
